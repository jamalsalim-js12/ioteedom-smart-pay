...
import { paymentStatusClass, ReceiptDocket } from "@/components/ui/receipt-dialog";
...
export default function OpsPaymentDetailPage() {
  ...
  const method = payment
    ? (paymentMethods.find((item) => item.id === payment.method)?.name ?? payment.method)
    : "";
  ...
  if (!payment) {
    return (
      <div className="enter">
        ...
        <div className="p-6">
          <Panel>
            <EmptyState icon={Search} title="No payment here" body={`Nothing matches ${id}.`} />
          </Panel>
        </div>
      </div>
    );
  ...
  return (
    <div className="enter">
      <OpsTopbar backHref="/admin/payments" kicker={payment.ref} title={payment.label} />
          ...
          <Facts
            rows={[
              ...
              { label: "Time", value: payment.at },
              {
                label: "Property",
                value: (
                  <Link href={`/admin/accounts/${payment.propertyId}`} className="underline">
